...
pub fn qgcrf2itrf(epoch: Epoch) -> DQuat {
    rotation(EARTH_J2000, EARTH_ITRF93, epoch)
}

/// ITRF -> GCRF, apply as `q * v`. Positions only (see [`qgcrf2itrf`]).
pub fn qitrf2gcrf(epoch: Epoch) -> DQuat {
    rotation(EARTH_ITRF93, EARTH_J2000, epoch)
}
...
pub fn qteme2gcrf(epoch: Epoch) -> DQuat {
    rotation(EARTH_TEME_LEGACY_FRAME, EARTH_J2000, epoch)
}

/// TEME -> ITRF, apply as `q * v`. Positions only (see [`qgcrf2itrf`]).
pub fn qteme2itrf(epoch: Epoch) -> DQuat {
    rotation(EARTH_TEME_LEGACY_FRAME, EARTH_ITRF93, epoch)
}
...
fn rotation(from: Frame, to: Frame, epoch: Epoch) -> DQuat {
    let dcm = context()
        .almanac
