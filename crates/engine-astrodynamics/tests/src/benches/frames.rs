...
//! prepared outside the timed closures and both data stacks warm up
//! before measurement (seeding here, lazy kernel parse in warm-up).
...
fn bench_frames(c: &mut Criterion) {
    ...
    group.bench_function("crate", |b| {
        b.iter(|| frames::qgcrf2itrf(black_box(epoch)));
    });
    ...
    group.bench_function("crate", |b| {
        b.iter(|| frames::qteme2gcrf(black_box(epoch)));
    });
    group.bench_function("satkit", |b| {
