//! Physical constants and geometry for the Moon, in real-world units - the
//! lunar twin of [`crate::earth`].
//!
//! World space throughout the renderer is **kilometers**; this module gives the
//! Moon's body-fixed (selenographic) surface points and normals in the same
//! parameterization convention as `earth`: **+Y is north** (the lunar rotation
//! pole), selenographic longitude 0 deg / latitude 0 deg (the mean sub-Earth
...
//!
//! Unlike Earth's WGS84 (an oblate spheroid of revolution), the Moon is modeled
//! as a **triaxial** ellipsoid - its three principal axes differ. The long axis
//! points at Earth (tidal bulge), the short axis is the rotation pole. The
...
/// IAU/IAG mean radius along the principal axis pointing at Earth (the
/// sub-Earth/prime-meridian direction, +Z here), km. The longest axis.
pub const RADIUS_SUBEARTH_KM: f64 = 1737.4;
...
/// (the Moon as a shadow caster is treated as a sphere of this radius - the
...
/// Semi-axis along world +Z (sub-Earth / prime meridian).
const RZ_KM: f64 = RADIUS_SUBEARTH_KM;
...
/// parameterization matches the equirectangular texture exactly as Earth's
/// does.
...
/// radial direction; with the Moon's near-sphericity it is within ~0.04 deg of
