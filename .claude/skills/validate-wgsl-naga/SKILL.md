...
description: Statically validate engine/src/shaders/scene.wgsl with the naga CLI (naga --compact --capabilities none). This is the authoritative shader check - the same naga the app links through wgpu. Use after every shader edit; a clean cargo build says nothing about the shader.
---

# Validate WGSL with the naga CLI (authoritative)

Statically validate `engine/src/shaders/scene.wgsl` after **every** shader edit. This
