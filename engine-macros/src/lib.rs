...
///
/// Emits `::engine::scene::{SceneClock, Clock}` paths, so it only works
/// inside a crate that depends on the `engine` crate under that name — which
/// is every user this repo-internal crate will ever have.
...
pub fn derive_scene_clock(input: TokenStream) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::engine::scene::SceneClock for #name #ty_generics #where_clause {
            fn clock_mut(&mut self) -> &mut ::engine::scene::Clock {
                &mut self.clock
            }
        }
    }
...
pub fn derive_scene_orbital_bodies(input: TokenStream) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = input.generics.split_for_impl();
    quote! {
        impl #impl_generics ::engine::scene::SceneOrbitalBodies for #name #ty_generics #where_clause {
            fn orbital_bodies_mut(
                &mut self,
            ) -> &mut [::engine::scene::orbital_body::OrbitalBody] {
                #body
            }
        }
    }
...
pub fn derive_scene_kinematic_bodies(input: TokenStream) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = input.generics.split_for_impl();
    quote! {
        impl #impl_generics ::engine::scene::SceneKinematicBodies for #name #ty_generics #where_clause {
            fn kinematic_bodies_mut(
                &mut self,
            ) -> &mut [::engine::scene::kinematic_body::KinematicBody] {
                #body
            }
        }
    }
...
pub fn derive_scene_ptz_camera(input: TokenStream) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = input.generics.split_for_impl();
    quote! {
        impl #impl_generics ::engine::camera::ScenePtzCamera for #name #ty_generics #where_clause {
            fn camera(&self) -> &::engine::camera::PtzCamera {
                &self.camera
            }

            fn camera_mut(&mut self) -> &mut ::engine::camera::PtzCamera {
                &mut self.camera
            }

            fn camera_target(&self) -> &::engine::scene::CameraTarget {
                &self.camera_target
            }
        }
    }
